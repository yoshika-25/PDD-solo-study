...
if ($action == 'add') {
    ...
    $sql = "INSERT INTO que (question, option_a, option_b, option_c, option_d, correct_option) VALUES (?, ?, ?, ?, ?, ?)";
...
elseif ($action == 'update') {
    ...
    $sql = "UPDATE que SET question=?, option_a=?, option_b=?, option_c=?, option_d=?, correct_option=? WHERE id=?";
    $stmt = $conn->prepare($sql);
...
// Fetch all questions
$sql = "SELECT id, question, option_a, option_b, option_c, option_d, correct_option FROM que";
...
<head>
    ...
    <style>
        body {
            background: linear-gradient(45deg, #ff758c, #ff7eb3, #ff96a1, #ff758c);
